...
\end{equation}
上式表明，剪切应力约束由两部分组成：其一为域内剪切力所应满足的散度平衡条件 $\nabla\cdot\mathbf{q}^{h}+\bar{p}=0$，其二为自然边界 $\Gamma_q$ 上的剪切力条件 $\mathbf{q}^{h}\cdot\mathbf{n}=\bar{q}_n$，二者共同反映了剪切力的力平衡关系。由此可见，该约束在本质上是剪切力的力平衡条件。
...
\end{equation}
上式表明，剪切应变约束由两部分组成：其一为域内剪切应变应满足的协调条件 $\nabla w^{h}-\boldsymbol{\varphi}^{h}=\mathbf{0}$，即剪切应变 $\boldsymbol{\gamma}=\mathbf{0}$；其二为本质边界 $\Gamma_w$ 上的挠度条件 $w^{h}=\bar{w}$。二者共同反映了板的变形协调关系，因此该约束在本质上是剪切应变的变形协调条件。

由上可见，式~\eqref{eq:weak-w-h} 与式~\eqref{eq:weak-q-h} 是两个物理本质不同的约束，因而分别需要各自的LBB稳定性条件。值得注意的是，传统免自锁方法大多仅针对式~\eqref{eq:weak-q-h} 所示的剪切应变约束进行构造，而对式~\eqref{eq:weak-w-h} 中剪切应力约束的稳定性关注甚少，这也正是部分方案在满足单一LBB条件后剪切力仍出现振荡的原因所在。
